...
public class Horas extends PathfinderMob implements FlyingAnimal {
    private float flap, flapSpeed, flapping = 1.0F;
    private Player playerEntity;
    private final PathNavigation navigator;
    private int timeToRecalcPath;

    public Horas(EntityType<? extends Horas> entityType, Level world) {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, MobSpawnType reason, SpawnGroupData spawnDataIn, CompoundTag dataTag) {
        if (reason == MobSpawnType.MOB_SUMMONED || reason == MobSpawnType.SPAWN_EGG) this.playerEntity = worldIn.getNearestPlayer(this, 10D);
