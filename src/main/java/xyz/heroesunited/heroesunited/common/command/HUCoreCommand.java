...
public class HUCoreCommand {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands.literal("heroesunited").requires((player) -> player.hasPermission(2))
                .then(Commands.literal("slowmo").then(Commands.argument("enable", FloatArgumentType.floatArg()).executes((c) -> setSlowMotion(c.getSource(), FloatArgumentType.getFloat(c, "amount")))))
                ...
                .then(Commands.literal("superpower")
                        .then(Commands.literal("level").then(Commands.argument("players", EntityArgument.players())
                                .then(Commands.argument("value", IntegerArgumentType.integer())).executes(c -> setSuperpowerLevel(c.getSource(), EntityArgument.getPlayers(c, "players"), IntegerArgumentType.getInteger(c, "value")))))
                        .then(Commands.argument("players", EntityArgument.players())
