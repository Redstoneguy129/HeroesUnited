...
public class HULayerRenderer<T extends LivingEntity, M extends BipedEntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    @Override
    public void render(MatrixStack matrixStack, VertexConsumerProvider buffer, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (entityRendererIn instanceof PlayerEntityRenderer && entity instanceof AbstractClientPlayerEntity) {
            PlayerEntityRenderer playerRenderer = (PlayerEntityRenderer) entityRendererIn;
            ...
            player.getCapability(HUPlayerProvider.CAPABILITY).ifPresent(cap -> {
                for (int slot = 0; slot < cap.getInventory().getContainerSize(); ++slot) {
                    ...
                    if (stack != null && stack.getItem() instanceof IAccessory && !MinecraftForge.EVENT_BUS.post(new HURenderLayerEvent.Accessories(playerRenderer, player, matrixStack, buffer, packedLight, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch))) {
                        ...
                        /**@TO-DO Scale for Accessories  `accessoire.getScale(stack)` */
    ...
    private void renderAccessories(SuitModel suitModel, MatrixStack matrixStack, VertexConsumerProvider buffer, int packedLight, PlayerEntity player, PlayerEntityRenderer playerRenderer, IAccessory accessoire, ItemStack stack, IHUPlayer cap, EquipmentAccessoriesSlot slot) {
        ...
        suitModel.render(matrixStack, buffer.getBuffer(RenderLayer.getEntityTranslucent(accessoire.getTexture(stack, player, slot))), packedLight, OverlayTexture.DEFAULT_UV, 1f, 1f, 1f, 1f);
    }

    private void renderSuit(MatrixStack stack, VertexConsumerProvider buffer, T entity, EquipmentSlot slot, int packedLight, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
